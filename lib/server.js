...
module.exports = class Server extends EventEmitter {
  ...
  _createHttpServer() {
    return new Promise((resolve, reject) => {
      ...
      if (this.opts.secure) {
        ...
        childProcess.execFile('sslfie', ['--o' + path.join(__dirname, 'crt/server.crt'), '--k' + path.join(__dirname, 'crt/server.key'), this.opts.host], (error, stdout, stderr) => {
          if (error) {
            console.log('Error occured in sslfie child stderr :', stderr);
            throw error;
          }
          if (stdout.length) {
            console.log('Something occured in sslfie child stdout :', stdout);
          }
          console.log('certificate created for ' + this.opts.host + ' : ' + path.join(__dirname, 'crt/server.crt'));
          ...
          if (this.opts.http2) {
            resolve(http2.createServer(CERT_INFO, this._app).listen(this.opts.port, () => this._serverAlive()));
          } else {
            resolve(https.createServer(CERT_INFO, this._app).listen(this.opts.port, () => this._serverAlive()));
          }

        });
      } else {
        resolve(http.createServer(this._app).listen(this.opts.port, () => this._serverAlive()));
      }
    });
  }
  ...
  _serverAlive() {
    ...
    console.info(type + ' server alive, running on : ' + url);
  }

  _createServer() {
    // Check if one port is below 1024
    // see https://www.w3.org/Daemon/User/Installation/PrivilegedPorts.html & https://www.google.fr/search?q=node+ports+below+1024
    if (this.opts.port <= 1024 || (this.opts.redirectHttpToHttps && this.opts.insecurePort <= 1024)) {
      ...
      console.info(chalk.yellow('you are using the port ' + port + ' which is below or equal to 1024' + "\n" + 'start this script with elevated privileges to avoid EACCESS errors'));
    }

    // Create the server instance
    this._createHttpServer().then(server => {
    ...
    if (this.opts.redirectHttpToHttps) {
      console.info(chalk.green('will redirect http (' + this.opts.insecurePort + ') traffic to https (' + this.opts.port + ')'));
